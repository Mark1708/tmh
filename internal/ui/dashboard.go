...
type dashboardModel struct {
	...
	// paneBaseOffset is added to loop indices when constructing pane cache keys
	// so that the key matches what tmux reports when pane-base-index != 0.
	paneBaseOffset int
}
...
// SetPaneBaseIndex configures the pane offset used when building cache keys.
// Pass the value of tmux's pane-base-index option (typically 0 or 1).
func (d *dashboardModel) SetPaneBaseIndex(base int) {
	if d.paneBaseOffset != base {
		d.paneBaseOffset = base
		d.rebuildRows()
	}
}
...
func (d *dashboardModel) currentPreviewTarget() string {
	...
	case levelSession:
		return r.Session
	case levelWindow:
		return fmt.Sprintf("%s:%d.%d", r.Session, r.WindowIdx, d.previewPaneIdx+d.paneBaseOffset)
	case levelPane:
