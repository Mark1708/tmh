...
- наличие tmux-сервера, опциональных `fd`, `terminal-notifier` (только на macOS),
...
Убедись что используешь Go ≥ 1.25 и путь `github.com/mark1708/tmh/cmd/tmh@latest`. Если модуль недоступен через `proxy.golang.org` — установи `GOPROXY=direct`.
