...
| Конфиг | `~/.config/tmh/config.yml`, JSON Schema в [`schemas/tmh.schema.json`](./schemas/tmh.schema.json) |
| Основные режимы | fuzzy picker, полный TUI-дашборд, scripted CLI |
| Базовый workflow | описать YAML → `tmh init` → `tmh diff` → `tmh freeze` |
| Сгенерированная документация | man-страницы и shell completions в [`docs/`](./docs/) |
...
- [Правила discovery (glob + zoxide)](#правила-discovery-glob--zoxide)
...
tmh attach atlas:web             # вне tmux → attach
...
## Обмен конфигурацией с коллегой
